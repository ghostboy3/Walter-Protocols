def get_values(*names):
            import json
            _all_values = json.loads("""{"num_samp":8,"digest_time_hours":12,"bead_incubation":15,"test_mode":false,"p50_mount":"right","m50_mount":"left","protocol_filename":"micro_bio_id"}""")
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    for starting_sample, dest in zip(start_sample_plate.rows()[0][:num_col],
        ...
        pick_up(m50)
        for _ in range(2):
            if m50.current_volume > 0:
                m50.dispense(m50.current_volume, lysis_buffer)
            m50.aspirate(40, lysis_buffer)
            m50.air_gap(airgap)
            m50.dispense(40+airgap, starting_sample.top())
            m50.air_gap(airgap)
        m50.dispense(airgap, starting_sample.top())
        m50.mix(10, 40, starting_sample, rate=0.5)
        m50.aspirate(50, starting_sample.bottom(z=0.7), rate=0.3)
    ...
    for i, col in enumerate(samples):
        pick_up(m50)
        if i == 0:
            m50.mix(20, num_col*bead_vol*0.6 if num_col*bead_vol*0.6 < 50 else 50, beads.bottom(1.7), rate=0.3)
        else:
            m50.mix(5, num_col*bead_vol*0.6 if num_col*bead_vol*0.6 < 50 else 50, beads.bottom(1.7), rate=0.5)
        m50.aspirate(bead_vol, beads, rate=0.3)
    ...
    for s, d in zip(samples, dest_plate.rows()[0][:num_col]):
        pick_up(m50)
        m50.aspirate(15, s.bottom(z=0.8), rate=0.1)
        m50.aspirate(10, s.bottom(z=0.4), rate=0.1)
